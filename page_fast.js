...
async function fetchSheet(sid, gid){
  try{
    ...
    return t.split(NL).filter(l=>l.trim());
  }catch(e){return [];}
}

function parseDateRows(lines, colIdx){
  return lines.slice(4).filter(r=>{const c=r.split(",");return c[0]&&DR.test(c[0].replace(/"/g,""));}).map(r=>{
    const c=r.split(",");
    const tgl=c[0].replace(/"/g,"");
    if(colIdx===9){ // 2026/2025 format (11 cols)
      return{tgl,pc:p(c[1]),pm:p(c[2]),pq:p(c[3]),pj:p(c[4]),sc:p(c[5]),sm:p(c[6]),sq:p(c[7]),sj:p(c[8]),tt:p(c[9]),kj:p(c[10])};
    }else{ // 2024 format (5 cols)
      return{tgl,pg:p(c[1]),sg:p(c[2]),tt:p(c[3]),kj:p(c[4])};
    }
  });
}
...
export default function Home(){
  ...
  const [pj,setPj]=useState([]);const [p24,setP24]=useState([]);const [p25,setP25]=useState([]);
  const [fk,setFk]=useState([]);const [lp,setLp]=useState(null);
  const [lds,setLds]=useState([]);const [ml,setMl]=useState({26:[],25:[],24:[]});

  // Load only list of available months (lightweight)
  useEffect(()=>{(async()=>{
    try{
      const a=[];for(const m of P26M){const ll=await fetchSheet(S26,m.g);if(ll.length>4&&DR.test(ll[4].split(",")[0]?.replace(/"/g,"")))a.push(m.l);}
      const b={};for(const [gid,bln]of Object.entries(M24)){const ll=await fetchSheet(S24,gid);if(ll.length>4&&DR.test(ll[4].split(",")[0]?.replace(/"/g,"")))b[bln]=gid;}
      const c={};for(const [gid,bln]of Object.entries(M25)){if(bln==="DG")continue;const ll=await fetchSheet(S25,gid);if(ll.length>4&&DR.test(ll[4].split(",")[0]?.replace(/"/g,"")))c[bln]=gid;}
      setMl({26:a,25:c,24:b});
      if(a.length>0)setBA(a[a.length-1]);
      const ck=Object.keys(c);if(ck.length>0)setB25(ck[0]);
      const bk=Object.keys(b);if(bk.length>0)setB24(bk[0]);

      // Load LPH dates list
      const lm={};let firstLd="";
      for(const gid of LG){const ll=await fetchSheet(LSID,gid);const tg=ll[2]?.split(",")[3]?.replace(/"/g,"")?.trim()||"";if(!tg)continue;if(!firstLd)firstLd=tg;const rr=[];for(const l of ll){const c=l.split(",").map(x=>x.replace(/"/g,"").trim());const u=c[0].toUpperCase().trim();if(LU.some(v=>u===v||u.startsWith(v)))rr.push({u:c[0],p:c[1]||"-",s:c[2]||"-",t:c[3]||"-"});}if(rr.length>0)lm[tg]={tg,rr};}
      const ds=Object.keys(lm).sort();setLds(ds);
      if(ds.length>0){setLD(ds[ds.length-1]);setLp(lm[ds[ds.length-1]]);}
    }catch(e){setE("Err: "+e.message);}setL(false);
  })();},[]);

  // Fetch 2026 data when BA changes
  useEffect(()=>{(async()=>{if(!BA)return;const idx=P26M.findIndex(m=>m.l===BA);if(idx<0)return;const ll=await fetchSheet(S26,P26M[idx].g);setPj(parseDateRows(ll,9));})();},[BA]);

  // Fetch 2024 data when B24 changes
  useEffect(()=>{(async()=>{if(!B24)return;const gid=M24[B24];if(!gid)return;const ll=await fetchSheet(S24,gid);setP24(parseDateRows(ll,3));})();},[B24]);

  // Fetch 2025 data when B25 changes
  useEffect(()=>{(async()=>{if(!B25)return;const gid=M25[B25];if(!gid)return;const ll=await fetchSheet(S25,gid);setP25(parseDateRows(ll,9));})();},[B25]);

  // Fetch Faktur when FB changes
  useEffect(()=>{(async()=>{if(!FB)return;const m=FM.find(x=>x.l===FB);if(!m)return;const ll=await fetchSheet(FSID,m.g);
  ...
  // Fetch LPH when LD changes
  useEffect(()=>{(async()=>{if(!LD)return;for(const gid of LG){const ll=await fetchSheet(LSID,gid);const tg=(ll[2]?.split(",")[3]?.replace(/"/g,"")?.trim()||"");if(tg===LD){const rr=[];for(const l of ll){const c=l.split(",").map(x=>x.replace(/"/g,"").trim());const u=c[0].toUpperCase().trim();if(LU.some(v=>u===v||u.startsWith(v)))rr.push({u:c[0],p:c[1]||"-",s:c[2]||"-",t:c[3]||"-"});}setLp({tg,rr});return;}}
    setLp(null);})();},[LD]);

  const tt26=pj.reduce((s,d)=>s+d.tt,0);
  const tt24=p24.reduce((s,d)=>s+d.tt,0);
  const tt25=p25.reduce((s,d)=>s+d.tt,0);
...
<p style={{color:"rgba(255,255,255,.4)",fontSize:11}}>Mohon tunggu beberapa saat</p>
<style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style></div>;
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f172a 0%,#1e3a5f 40%,#1e40af 60%,#2563eb 100%)"}}>
    ...
    <div style={{padding:16,maxWidth:1200,margin:"0 auto"}}>
      {T==="p"&&<Penjualan BA={BA} setBA={setBA} ml={ml} pj={pj} tt26={tt26}
        B24={B24} setB24={setB24} p24={p24} tt24={tt24}
        B25={B25} setB25={setB25} p25={p25} tt25={tt25}/>}
      {T==="l"&&<LPH lds={lds} LD={LD} setLD={setLD} lp={lp}/>}
      {T==="f"&&<Faktur Fkeys={Object.keys(FM.map(m=>m.l).reduce((a,l)=>({...a,[l]:true}),{}))} FB={FB} setFB={setFB} fk={fk}/>}
    </div>
  </div>;
}

function Penjualan({BA,setBA,ml,pj,tt26,B24,setB24,p24,tt24,B25,setB25,p25,tt25}){
  return <div>
...
{(ml[26]||[]).map(b=><option key={b} value={b}>{b}</option>)}</select>
...
{Object.keys(ml[25]||{}).map(b=><option key={b} value={b}>{b}</option>)}</select>
...
{Object.keys(ml[24]||{}).map(b=><option key={b} value={b}>{b}</option>)}</select>
</div>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:20}}>
<K label={"2026 "+BA} v={fr(tt26)} c="#f59e0b"/><K label={"2025 "+B25} v={fr(tt25)} c="#3b82f6"/><K label={"2024 "+B24} v={fr(tt24)} c="#10b981"/>
</div>
{Array.isArray(pj)&&pj.length>0&&<Tabel title={"Penjualan Harian 2026 - "+BA} data={[...pj].reverse()} cols={["tgl","tt","kj"]} fmt={(k,v)=>k==="tt"?fr(v):v}/>}
{Array.isArray(p25)&&p25.length>0&&<Tabel title={"Penjualan Harian 2025 - "+B25} data={[...p25].reverse()} cols={["tgl","tt","kj"]} fmt={(k,v)=>k==="tt"?fr(v):v}/>}
{Array.isArray(p24)&&p24.length>0&&<Tabel title={"Penjualan Harian 2024 - "+B24} data={[...p24].reverse()} cols={["tgl","tt","kj"]} fmt={(k,v)=>k==="tt"?fr(v):v}/>}
</div>;}
...
function Faktur({Fkeys,FB,setFB,fk}){
  ...
  return <div>
<div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>{FM.map(m=><button key={m.l} onClick={()=>setFB(m.l)} style={bs(FB,m.l)}>{m.l}</button>)}</div>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:20}}>
<K label={"Faktur "+FB} v={fk.length+" faktur"} c="#3b82f6"/><K label="Lunas" v={fk.filter(d=>d.sb).length+" faktur"} c="#10b981"/><K label="Belum" v={fk.filter(d=>!d.sb).length+" faktur"} c="#ef4444"/><K label="Nilai" v={fr(fk.reduce((s,d)=>s+d.jml,0))} c="#8b5cf6"/>
</div>
